...
def regolamento(call):
    try:
        ...
        full = types.InlineKeyboardButton(text='Leggi di più 🎓', url='https://telegra.ph/Regole-del-gruppo-10-07')
        tastiera.add(full)
        indietro = types.InlineKeyboardButton(text='Indietro 🔙', callback_data='rugoles')
        tastiera.add(indietro)
        ...
        salvaerrore(ex)
@bot.callback_query_handler(func=lambda c: c.data == 'rugoles')
def rugoles(call):
    try:
...
def indietro(call):
    eliminaspoiler(call.from_user.id)
    tastiera = types.InlineKeyboardMarkup()
    spoiler = types.InlineKeyboardButton(text="Spoiler 🔒", callback_data="spoiler")
    tastiera.add(spoiler)
    try:
        eliminaspoiler(call.from_user.id)
        bot.edit_message_text(
            "👋 » <i>" + namechanger(call.from_user.first_name, call.from_user.id) + " grazie per avermi avviato!</i>",
            call.message.chat.id, call.message.message_id, reply_markup=tastiera, parse_mode="html")
    except Exception as ex:
        salvaerrore(ex)
        print(ex)
...
def chatblacklist(chat : str) : 
    ...
    verifica = str(chat)
    if verifica[0] == '-' and chat != gruppo and chat != canale_artehub and chat != canale_gruppo and chat != canale_log : 
