...
def ask(message):
    if chatblacklist(message.chat.id) is True : 
        try_to(message, "❓»<i>" + haimaiandaskgenerator(message, "ask") + "</i>")
...
def accettazione(message):
    try:
        bot.send_message(message.from_user.id,
                         "<b>Richiesta a Gruppo ita inviata 📫</b>\n\n<i>• No dc 1 (VoIP)\n• Se non hai la pic devi attendere accettazione dagli admin </i>",
                         parse_mode='html')
        if int(bot.get_user_profile_photos(message.from_user.id).total_count > 0):
            ...
            bot.send_message(message.from_user.id,
                             "<b>Non hai una foto profilo !</b>\n\n<i>• Attendi che gli admin ti accettino ⏳\n• Permetti al bot di visualizzare la foto profilo sulle impostazioni privacy 🔒\n• Metti una foto profilo e rimanda la richiesta 🚪\n\nUna volta passati i controlli potrai togliere la foto profilo</i>",
                             parse_mode='html')
            tastiera = types.InlineKeyboardMarkup()
