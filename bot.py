...
def addask(message): 
    if chatblacklist(message.chat.id) is True : 
        if contenuto := verifysecond(message, 'addask') is 'false': nontrovato(message, '/addask [domanda]')
...
def addask(message): 
    if chatblacklist(message.chat.id) is True : 
        if contenuto := verifysecond(message, 'addhaimai') is 'false' : nontrovato(message, '/addhaimai [haimai]')
        elif cercaoperatoredaid(message) is None : try_to(message, 'Devi essere operatore per svolgere questa operazione ❌')
        elif "#Addhaimai\n" in contenuto : try_to(message,"Hashtag riservato al log ❌")
...
def bagasciamometro(message):
    try:
        if chatblacklist(message.chat.id) is True : 
            try_to(message,
                "🎲 »<i> " + namechanger(message.from_user.first_name, message.from_user.id) + " è bagascia al " + str(
...
def coglionometro(message):
    try:
        if chatblacklist(message.chat.id) is True : 
            try_to(message,
                "🎲 »<i> " + namechanger(message.from_user.first_name, message.from_user.id) + " è coglione al " + str(
                    random.randint(0, 100)) + " % 🌵</i>")
    except Exception as ex:
        salvaerrore(ex)
...
def albanemometro(message):
    try:
        if chatblacklist(message.chat.id) is True : 
            try_to(message,
                "🎲 »<i> " + namechanger(message.from_user.first_name, message.from_user.id) + " è albanese al " + str(
                    random.randint(0, 100)) + " % 🇦🇱</i>")
    except Exception as ex:
        salvaerrore(ex)
...
def napolometro(message):
    try:
        if chatblacklist(message.chat.id) is True : 
            try_to(message,
                "🎲 »<i> " + namechanger(message.from_user.first_name, message.from_user.id) + " è napoletano al " + str(
                    random.randint(0, 100)) + " % 🍕</i>")
    except Exception as ex:
        salvaerrore(ex)
...
def sforna_bambini(xid, xname, yid, yname, call):
    try:
        ...
        bot.send_photo(call.message.chat.id, fota,caption="👼 » <i>" + namechanger(xname, xid) + " ha fatto un bambino con " + namechanger(yname,yid) +"\n🏷 »  "+str(name['name'])+" "+str(surname['name'])+"</i>",parse_mode="html")
        ...
        if id == False:
            ...
            try:
                if random.randint(0, 4) < 4:
                    ...
                    fota = str(photos[random.randint(0, 79)].original) + "?auto=compress&cs=tinysrgb&dpr=2&h=700&w=1200"
                    bot.send_photo(message.chat.id, fota, caption="👼 » <i>" + namechanger(message.from_user.first_name,message.from_user.id) + " ha fatto un bambino con " + namechanger(message.reply_to_message.from_user.first_name, message.reply_to_message.from_user.id) +"\n🏷 »  "+str(name['name'])+" "+str(surname['name'])+"</i>",parse_mode="html")
            except Exception as ex:
                salvaerrore(ex)
...
def stato(message):
    try:
        if chatblacklist(message.chat.id) is True : 
            ...
            try_to(message, "<b>Stato di " + namechanger(message.from_user.first_name,message.from_user.id) + "📊</b> \n" +
    ...
    if chatblacklist(message.chat.id) is True : 
        ...
        if contenuto == False or verifyvalue(message, contenuto) == False:
            ...
            if id == False:
                ...
                record2 = controlla_e_crea(message.reply_to_message.from_user.first_name,message.reply_to_message.from_user.id)
                if float(contenuto) > float(40000):
                    ...
                    try:
                        ...
                        dbstato.find_one_and_update({'id': message.reply_to_message.from_user.id},
                                                    {"$set": {'succhini': float(record2['succhini']) + float(contenuto)}},
                                                    upsert=True)
                    except Exception as ex:
                        salvaerrore(ex)
            ...
            if record1['succhini'] < float(contenuto):
                ...
                if random.randint(0, 10) < 7:
                    ...
                    dbstato.find_one_and_update({'id': message.from_user.id},
                                                ...
                                                upsert=True)
                    try_to(message,
                        "😔 <b> cavolo </b> » <i>Hai perso</i><code> " + display(contenuto) + " </code> <i>succhini</i>")
...
def meteo(message):
    if chatblacklist(message.chat.id) is True : 
        ...
        if contenuto == False:
            ...
            try:
                response = requests.get('http://api.openweathermap.org/data/2.5/weather?q=' + contenuto.replace(" ",
                                                                                                                "+") + '&units=Metric&APPID=cee33cf099c0cb10eb63d82dd1b47a18').json()
                if "city not found" in str(response):
                    try_to(message, "🗺 »<i> Luogo non trovato</i>")
                else:
                    if str(response['weather'][0]['main']) == "Clear":
                        descrizione = "Sereno ☀️"
                    elif str(response['weather'][0]['main']) == "Clouds":
                        descrizione = "Nuvoloso 🌤 "
                    elif str(response['weather'][0]['main']) == "Mist":
                        descrizione = "Foschia ❗️"
                    elif str(response['weather'][0]['main']) == "Smoke":
                        descrizione = "Fumo nell'aria 🚬"
                    elif str(response['weather'][0]['main']) == "Haze":
                        descrizione = "Caligine 💭"
                    elif str(response['weather'][0]['main']) == "Dust":
                        descrizione = "Vortici di sabbia/polvere ⏳"
                    elif str(response['weather'][0]['main']) == "Fog":
                        descrizione = "Nebbia 🌫"
                    elif str(response['weather'][0]['main']) == "Ash":
                        descrizione = "cenere vulcanica 🌋"
                    elif str(response['weather'][0]['main']) == "Squall":
                        descrizione = " Bufera ☃️"
                    elif str(response['weather'][0]['main']) == "Tornado":
                        descrizione = "Tornado 🌪"
                    elif str(response['weather'][0]['main']) == "Snow":
                        descrizione = "Nevicate ❄️"
                    elif str(response['weather'][0]['main']) == "Rain":
                        descrizione = "Pioggie 🌧"
                    elif str(response['weather'][0]['main']) == "Drizzle":
                        descrizione = "Pioggerella 🌦"
                    elif str(response['weather'][0]['main']) == "Thunderstorm":
                        descrizione = "Temporali ⛈"
                    tastiera = types.InlineKeyboardMarkup()
...
def accettazione(message):
    try:
        bot.send_video(message.from_user.id, open('sending.mp4','rb'), caption='𝙍𝙞𝙘𝙝𝙞𝙚𝙨𝙩𝙖 𝙖 𝙂𝙧𝙪𝙥𝙥𝙤 𝙞𝙩𝙖 𝙞𝙣𝙫𝙞𝙖𝙩𝙖 📫\n\n❌ 𝐍𝐨 𝐝𝐜 𝟏 (𝐕𝐨𝐈𝐏)\n\n⏳ 𝐒𝐞 𝐧𝐨𝐧 𝐡𝐚𝐢 𝐥𝐚 𝐩𝐢𝐜 𝐝𝐞𝐯𝐢 𝐚𝐭𝐭𝐞𝐧𝐝𝐞𝐫𝐞 𝐚𝐜𝐜𝐞𝐭𝐭𝐚𝐳𝐢𝐨𝐧𝐞 𝐝𝐚𝐠𝐥𝐢 𝐚𝐝𝐦𝐢𝐧', parse_mode='html')
        ...
        if int(bot.get_user_profile_photos(message.from_user.id).total_count > 0):
            ...
            bot.send_video(message.from_user.id, open('nopic.mp4','rb'), caption="𝙉𝙤𝙣 𝙝𝙖𝙞 𝙪𝙣𝙖 𝙛𝙤𝙩𝙤 𝙥𝙧𝙤𝙛𝙞𝙡𝙤 !\n\n🕔 𝐀𝐭𝐭𝐞𝐧𝐝𝐢 𝐜𝐡𝐞 𝐠𝐥𝐢 𝐚𝐝𝐦𝐢𝐧 𝐭𝐢 𝐚𝐜𝐜𝐞𝐭𝐭𝐢𝐧𝐨 \n\n✅🔒  𝐏𝐞𝐫𝐦𝐞𝐭𝐭𝐢 𝐚𝐥 𝐛𝐨𝐭 𝐝𝐢 𝐯𝐢𝐬𝐮𝐚𝐥𝐢𝐳𝐳𝐚𝐫𝐞 𝐥𝐚 𝐟𝐨𝐭𝐨 𝐩𝐫𝐨𝐟𝐢𝐥𝐨 𝐬𝐮𝐥𝐥𝐞 𝐢𝐦𝐩𝐨𝐬𝐭𝐚𝐳𝐢𝐨𝐧𝐢 𝐩𝐫𝐢𝐯𝐚𝐜𝐲 \n\n🔄 𝐌𝐞𝐭𝐭𝐢 𝐮𝐧𝐚 𝐟𝐨𝐭𝐨 𝐩𝐫𝐨𝐟𝐢𝐥𝐨 𝐞 𝐫𝐢𝐦𝐚𝐧𝐝𝐚 𝐥𝐚 𝐫𝐢𝐜𝐡𝐢𝐞𝐬𝐭𝐚 \n\n🗑 𝐔𝐧𝐚 𝐯𝐨𝐥𝐭𝐚 𝐩𝐚𝐬𝐬𝐚𝐭𝐢 𝐢 𝐜𝐨𝐧𝐭𝐫𝐨𝐥𝐥𝐢 𝐩𝐨𝐭𝐫𝐚𝐢 𝐭𝐨𝐠𝐥𝐢𝐞𝐫𝐞 𝐥𝐚 𝐟𝐨𝐭𝐨 𝐩𝐫𝐨𝐟𝐢𝐥𝐨", parse_mode="html")
            tastiera = types.InlineKeyboardMarkup()
...
def rugolamento(call):
    try:
        ...
        tastiera.add(indietro)
        bot.edit_message_text(chat_id = call.message.chat.id,text= "📜 𝗥𝗲𝗴𝗼𝗹𝗲 :<i> \n\n🔞 No porno\n🧟‍♂️ No gore\n📩 No spam\n🔒 No privati senza consenso</i>",message_id = call.message.message_id, parse_mode="html", reply_markup=tastiera)
    except Exception as ex:
        salvaerrore(ex)
...
def indietroz(call):
    try:
        ...
        if x.status == 'member' or x.status == 'administrator': 
            ...
            bot.edit_message_text(chat_id = call.message.chat.id, text= "👋 » <i>" + namechanger(call.from_user.first_name,call.from_user.id) + " grazie per avermi avviato!</i>",message_id=call.message.message_id,parse_mode="html", reply_markup=tastiera)
        else: 
            bot.send_message(chat_id = call.message.chat.id, text="👋 » <i>" + namechanger(call.from_user.first_name,call.from_user.id) + " grazie per avermi avviato!</i>",message_id=call.message.message_id,parse_mode="html")
    except Exception as ex:
        salvaerrore(ex)
        print(ex)
            ...
            if trova is not None :
                ...
                bot.edit_message_text(
                    call.message.text + "\n\n ❌ » 𝐮𝐭𝐞𝐧𝐭𝐞 𝐧𝐨𝐧 𝐚𝐩𝐩𝐫𝐨𝐯𝐚𝐭𝐨 𝐝𝐚 " + namechanger(call.from_user.first_name,
                                                                                          call.from_user.id),
                    canale_log, trova['message'], parse_mode="html")
                bot.send_video(trova['utente'], open('decline.mp4','rb'),caption= "<b>Non sei stato approvato su Gruppo ita ❌</b>",parse_mode='html')
                dbinfo.delete_many({'argomento': 'accettazione', 'utente': trova['utente']}) 
            elif group is not None: 
                ...
                bot.edit_message_text(
                    call.message.text + "\n\n ❌ » 𝐮𝐭𝐞𝐧𝐭𝐞 𝐧𝐨𝐧 𝐚𝐩𝐩𝐫𝐨𝐯𝐚𝐭𝐨 𝐝𝐚 " + namechanger(call.from_user.first_name,
                                                                                          call.from_user.id),
                    canale_log, group['message'], parse_mode="html")
                bot.send_video(group['utente'], open('decline.mp4','rb'),caption= "<b>𝐍𝐨𝐧 𝐬𝐞𝐢 𝐬𝐭𝐚𝐭𝐨 𝐚𝐩𝐩𝐫𝐨𝐯𝐚𝐭𝐨 ❌</b>",parse_mode='html')
...
def startpvt(message):
    try:
        ...
        if x.status == 'member' or x.status == 'administrator': 
            ...
            bot.send_message(message.chat.id, "👋 » <i>" + namechanger(message.from_user.first_name,message.from_user.id) + " grazie per avermi avviato!</i>",parse_mode="html", reply_markup=tastiera)
        else: 
            bot.send_message(message.chat.id, "👋 » <i>" + namechanger(message.from_user.first_name,message.from_user.id) + " grazie per avermi avviato!</i>",parse_mode="html")
    except Exception as ex:
        salvaerrore(ex)
        print(ex)
