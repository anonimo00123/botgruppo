...
@bot.edited_message_handler(commands=['prova', 'prova'], chat_types='supergroup')
@bot.message_handler(commands=['prvoa', 'prova'], chat_types='supergroup')
def startgetphoto(message): Thread(target=gtphoto, args=[message]).start()


def gtphoto(message):
    x = bot.get_state(message.from_user.id, message.chat.id)
